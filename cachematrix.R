## Put comments here that give an overall description of what your
## functions do

## Create CacheMatrix object with 4 function, that can store  matrix
## and its invert.
##
## The functions set() and get(x) are used to set or get matrix data.
## The function setinv(x) is used to cached the inverse of the matrix
## The funciton getinv() is used to retrieve cached value.
##
## In reality the object is a list of the 4 function that set set and
## acaess the stored value in parent environment.

makeCacheMatrix <- function(x = matrix()) {
        #inv <- NULL
        set <- function(y) {
                x   <<- y
                inv <<- NULL
        }
        
        get <- function() x
        setinv <- function(inverse) inv <<- inverse
        getinv <- function() inv
        list(set = set, get = get, setinv= setinv, getinv = getinv)
}


## Write a short comment describing this function

cacheSolve <- function(x) {
        ## Return a matrix that is the inverse of 'x'
        inv <- x$getinv()
        
        if(!is.null(inv)) {
                message("getting cached data")
                return(inv)
        }
        data <- x$get()
        inv <- solve(data)
        x$setinv(inv)
        inv
}
